...
//    this._pitch = -Math.PI / 4;
    ...
    this._cameraRotation = new THREE.Matrix4();
    this._cameraEuler = new THREE.Euler();
    this._cameraEuler.order='ZYX'; // not sure
  }
  
  clone(){
    let c = new Potree.View();
    c.up = this.up;
    c._cameraRotation = this._cameraRotation;
    c._cameraEuler = this._cameraEuler;
    c.yaw = this.yaw;
    c._pitch = this.pitch;
  ...
  get yaw() {
   return this._cameraEuler.x;
  }

  set yaw(angle) {
    this._cameraEuler.x=angle;
    let q=new THREE.Quaternion().setFromEuler(this._cameraEuler);
    this._cameraRotation.makeRotationFromQuaternion(q);
  }

  get pitch(){
   return this._cameraEuler.z;
  }
  
  set pitch(angle){
 //   this._pitch = Math.max(Math.min(angle, this.maxPitch), this.minPitch);
   this._cameraEuler.z=angle; 
    let q=new THREE.Quaternion().setFromEuler(this._cameraEuler);
    this._cameraRotation.makeRotationFromQuaternion(q);
  }

  get roll() {
    return this._cameraEuler.y;
  }

  set roll(angle) {
    this._cameraEuler.y=angle;
    let q=new THREE.Quaternion().setFromEuler(this._cameraEuler);
    this._cameraRotation.makeRotationFromQuaternion(q);
  }

  get direction(){
    let R=this._cameraRotation.elements;
    return new THREE.Vector3(R[8],R[9],R[10]);
  }
  
  set direction(R){
    this._cameraEuler.setFromRotationMatrix(R);
  }
  ...
  lookAt(t){
    ...
    this.radius = radius;
    this.direction = this._cameraRotation;
  }
...
Potree.Scene = class extends THREE.EventDispatcher{
  constructor(){
    ...
    this.fruscene = new THREE.Scene();
    this.scene = new THREE.Scene();
    this.scenePointCloud = new THREE.Scene();
    if (false) this.sceneBG = new THREE.Scene();
  ...
  initialize(){
    ...
    if (this.grid) this.scene.add(grid);
...
//------------------------------------------------------------------------------------
  ...
  update(delta, timestamp){
    ...
    if(this.controls !== null){
      ...
      camera.quaternion.setFromRotationMatrix(this.scene.view._cameraRotation);
      var R=this.scene.view._cameraRotation.elements;
//      camera.lookAt(camera.position,new THREE.Vector3(R[8],R[9],R[10]),new THREE.Vector3(R[4],R[5],R[6]));
      camera.up.set(R[4],R[5],R[6]);
      camera.lookAt(this.scene.view.direction);
      camera.updateMatrixWorld();
...
class PotreeRenderer{
  ...
  render(){
    ...
    if(viewer.background === "skybox"){
      ...
      //viewer.renderer.clear(true, true, false);
      if (viewer.scene.sceneBG) viewer.renderer.render(viewer.scene.sceneBG, viewer.scene.cameraBG);
